# The First Sliver (Historic Brawl)

**Commander**
1 The First Sliver

**Deck**
// --- Cascade & Cheat Engines (Replacements for missing cards) ---
1 Etali, Primal Conqueror // Replaces Enigma Sphinx
1 Imoti, Celebrant of Bounty // Replaces Sweet-Gum Recluse
1 Trumpeting Carnosaur // Replaces Etherium-Horn Sorcerer

// --- Original Creature Base ---
1 Annoyed Altisaur
1 Apex Devastator
1 Atraxa, Grand Unifier
1 Aurora Phoenix
1 Boarding Party
1 Emrakul, the Promised End
1 Enlisted Wurm
1 Ethersworn Sphinx
1 Maelstrom Colossus
1 Maelstrom Wanderer
1 Sakashima's Protege
1 Stormscale Scion
1 Ulamog, the Ceaseless Hunger
1 Ugin, the Spirit Dragon

// --- Spells & Interaction ---
1 Brilliant Ultimatum
1 Call Forth the Tempest
1 Creative Technique
1 Dance with Calamity
1 Dragonstorm
1 Emergent Ultimatum
1 Hit the Mother Lode
1 Meteoric Mace
1 Omniscience
1 One with the Multiverse
1 Summon: Bahamut
1 Talon Gates of Madara
1 Tibalt's Trickery
1 Ingenuity Engine

// --- Ramp & Rocks ---
1 Arcane Signet
1 Chromatic Lantern
1 Coldsteel Heart
1 Relic of Legends
1 The Great Henge
1 Ugin's Labyrinth

// --- Arena Mana Base (5-Color Optimized) ---
1 Command Tower
1 Mana Confluence
1 Cavern of Souls
1 The World Tree
1 Plaza of Heroes
1 Fabled Passage
1 Indatha Triome
1 Ketria Triome
1 Raugrin Triome
1 Savai Triome
1 Zagoth Triome
1 Jetmir's Garden
1 Raffine's Tower
1 Spara's Headquarters
1 Xander's Lounge
1 Ziatora's Proving Ground
1 Blood Crypt
1 Breeding Pool
1 Godless Shrine
1 Hallowed Fountain
1 Overgrown Tomb
1 Sacred Foundry
1 Steam Vents
1 Stomping Ground
1 Temple Garden
1 Watery Grave
1 Polluted Delta
1 Flooded Strand
1 Windswept Heath
1 Wooded Foothills
1 Bloodstained Mire
1 Arid Mesa
1 Scalding Tarn
1 Misty Rainforest
1 Verdant Catacombs
1 Marsh Flats
1 Snow-Covered Plains
1 Snow-Covered Island
1 Snow-Covered Swamp
1 Snow-Covered Mountain
1 Snow-Covered Forest
1 Boseiju, Who Endures
1 Otawara, Soaring City
1 Sokenzan, Crucible of Defiance
1 Takenuma, Abandoned Mire
1 Eiganjo, Seat of the Empire

// --- Notes ---
// 1. Replaced 'Sweet-Gum Recluse', 'Etherium-Horn Sorcerer', and 'Enigma Sphinx' (Not on Arena)
//    with 'Imoti', 'Trumpeting Carnosaur', and 'Etali, Primal Conqueror'.
// 2. Replaced invalid 'Mercadian Masques' storage lands with a full Fetch/Shock/Triome base.
// 3. Tibalt's Trickery IS legal in Historic Brawl (only banned in Historic Bo1).