# Alela, Cunning Conqueror: Faerie & Rogue Tribal (Budget)

This deck is built for Jamie, focusing on **Faerie and Rogue tribal synergies** with a heavy emphasis on **Flash and Goad mechanics**. It adheres to the "New Era" template (38/10/12/12/6/31) and is designed for **Bracket 2** gameplay with a total cost under $100.

## 🏹 Strategy & Playstyle

*   **Stick and Protect:** Alela wants you to cast spells on *each* of your opponents' turns. This creates 1/1 Faerie Rogue tokens and triggers her goad ability.
*   **Political Goading:** Use your tokens to poke opponents. Alela forces their largest threats to attack each other, keeping the heat off you.
*   **The Finisher:** **Shadow Puppeteers** is the secret weapon. It turns your swarm of 1/1 tokens into 4/4 Dragons with Flying, ending the game quickly.
*   **Tempo Interaction:** You have a high count of instant-speed removal and counterspells to disrupt opponents while simultaneously advancing your board state.

## 📊 The "New Era" Template Breakdown

| Category | Count | Status |
| :--- | :---: | :--- |
| **Lands** | 38 | ✅ Optimized Budget Mana Base |
| **Ramp** | 10 | ✅ Sol Ring, Signets, Talisman |
| **Card Advantage** | 12 | ✅ Nymris, Wavebreak Hippocamp, Bident |
| **Targeted Disruption** | 12 | ✅ Counterspell, Infernal Grasp, Reality Shift |
| **Mass Disruption** | 6 | ✅ Aetherize, Perplexing Test (Token Friendly) |
| **Plan Cards** | 31 | ✅ Faeries, Rogues, Shadow Puppeteers |
| **Commander** | 1 | ✅ Alela, Cunning Conqueror |

---

## 🃏 Plain Text Copy/Paste (Moxfield Import)
*Note: Each line below ends with two spaces for GitHub Flavored Markdown support.*

COMMANDER:  
1 Alela, Cunning Conqueror  

DECK:  
1 Arcane Signet  
1 Dimir Signet  
1 Talisman of Dominance  
1 Sol Ring  
1 Mind Stone  
1 Thought Vessel  
1 Fellwar Stone  
1 Midnight Clock  
1 Wayfarer's Bauble  
1 Liquimetal Torque  
1 Brainstorm  
1 Opt  
1 Consider  
1 Wavebreak Hippocamp  
1 Nymris, Oona's Trickster  
1 Archmage Emeritus  
1 Bident of Thassa  
1 Reconnaissance Mission  
1 Coastal Piracy  
1 Fact or Fiction  
1 Distant Melody  
1 Frantic Search  
1 Counterspell  
1 Negate  
1 Infernal Grasp  
1 Reality Shift  
1 Rapid Hybridization  
1 Feed the Swarm  
1 Drown in the Loch  
1 Faerie Fencing  
1 Resculpt  
1 Cast Down  
1 Snap  
1 Unsubstantiate  
1 Aetherize  
1 Aetherspouts  
1 Perplexing Test  
1 Crippling Fear  
1 Curse of the Swine  
1 Flood of Tears  
1 Shadow Puppeteers  
1 Obyra, Dreaming Duelist  
1 Tegwyll, Duke of Splendor  
1 Scion of Oona  
1 Spellstutter Sprite  
1 Rankle, Master of Pranks  
1 Oona, Queen of the Fae  
1 Blightwing Bandit  
1 Anowon, the Ruin Thief  
1 Faerie Seer  
1 Mocking Sprite  
1 Quickling  
1 Pestermite  
1 Cloud of Faeries  
1 Faerie Vandal  
1 Malleable Impostor  
1 Sower of Temptation  
1 Archmage of Echoes  
1 Talion's Messenger  
1 Faerie Formation  
1 Stolen by the Fae  
1 Feywild Visitor  
1 Cloak and Dagger  
1 Sword of the Animist  
1 Skullclamp  
1 Oona's Blackguard  
1 Command Tower  
1 Dimir Aqueduct  
1 Tainted Isle  
1 Sunken Hollow  
1 Choked Estuary  
1 Temple of Deceit  
1 Exotic Orchard  
1 Myriad Landscape  
1 Path of Ancestry  
1 Rogue's Passage  
1 Bojuka Bog  
1 Reliquary Tower  
1 Evolving Wilds  
1 Terramorphic Expanse  
1 Ash Barrens  
11 Island  
11 Swamp  

## 🛠️ Notable Budget Swaps (Price-Checked)

| Original (Expensive) | Budget Replacement | Estimated Savings |
| :--- | :--- | :--- |
| **Bitterblossom** ($31) | **Feywild Visitor** ($0.25) | **$30.75** |
| **Faerie Mastermind** ($28) | **Nymris, Oona's Trickster** ($0.32) | **$27.68** |
| **Sword of Feast & Famine** ($36) | **Sword of the Animist** ($5.40) | **$30.60** |
| **Morphic Pool** ($24) | **Tainted Isle / Sunken Hollow** ($0.50) | **$23.50** |
| **Otawara, Soaring City** ($23) | **Island** ($0.15) | **$22.85** |
| **Deadly Rollick** ($22) | **Rapid Hybridization** ($0.20) | **$21.80** |
| **Polluted Delta** ($18) | **Evolving Wilds** ($0.15) | **$17.85** |
| **Grim Hireling** ($17) | **Prosperous Thief** ($0.22) | **$16.78** |
| **Black Market Connections** ($14) | **Coastal Piracy** ($1.00) | **$13.00** |

## 💰 Further Budget Optimization (Optional)

If Jamie needs to cut even more from the current $80 list, consider these "Luxury Budget" cuts:
1.  **Spellstutter Sprite ($6.00):** While a staple, she can swap this for **Counterspell ($1.75)** or **Negate ($0.25)** to save $4-5.
2.  **Reliquary Tower ($3.45):** In a budget deck without massive draw-X spells, a **Basic Land ($0.15)** is often better to ensure mana consistency.
3.  **Tegwyll, Duke of Splendor ($9.68):** Our most expensive non-commander creature. If needed, swap for **Obyra, Dreaming Duelist ($0.36)** for a similar tribal payoff.
4.  **Blightwing Bandit ($9.99):** Great for card advantage, but can be replaced with **Distant Melody ($0.22)** for a one-time burst.

---

## 📝 Deck Changelog

### 2026-02-21
*   Initial deck creation.
*   Focused on a sub-$100 budget for Jamie.
*   Included **Shadow Puppeteers** as requested.
*   **V2 Update:** Refined budget swaps using Manapool.com reference pricing ($500 shell reduced to ~$80).
*   Verified against **Bracket 2** rules (No "Game Changers" or infinite combos).
