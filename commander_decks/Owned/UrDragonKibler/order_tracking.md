# Order Tracking: Ur-Dragon (Order #241252 — Manapool)

**Order Date:** 2026-03-14
**Status:** 1/1 Packages Received — ✅ COMPLETE

---

## 📦 Package #241252-884082: Spellfinder
**Status:** ✅ Arrived
**Items:**
- [x] Mirari's Wake (MH2 320) — NM Borderless — $11.81
- [x] Birds of Paradise (DMR 439) — LP Borderless — $11.36
- [x] Terror of the Peaks (OTJ 337) — Foil NM Extended Art — $44.76
- [x] Bloom Tender (LRW-E 166) — NM — $13.08

---

# Order Tracking: Ur-Dragon (Order #223065)

**Order Date:** 2026-03-02  
**Status:** 10/10 Packages Received — ✅ COMPLETE

---

## 📦 Package: CardboardShuffle
**Status:** ✅ Arrived
**Items:**
- [x] Anguished Unmaking (2X2 170)

---

## 📦 Package 820930: FIVE6GAMING
**Status:** ✅ Arrived
**Items:**
- [x] Wooded Foothills (KTK 249)
- [x] Dromoka, the Eternal (FRF 151)
- [x] Misty Rainforest (MH2 477)
- [x] Miirym, Sentinel Wyrm (CLB 284)
- [x] Herald's Horn (LTC 280)
- [x] Karplusan Forest (BLC 314)
- [x] Arcane Signet (FDC 1)
- [x] Temple Garden (ECL 268)
- [x] Negate (TMT 47)

---

## 📦 Package 820928: HoiPolloi
**Status:** ✅ Arrived  
**Items:**
- [x] Utvara Hellkite (RTR 110)
- [x] Sylvan Caryatid (THS 180)
- [x] Atarka, World Render (PFRF 149★)
- [x] Llanowar Wastes (ORI 248)
- [x] Living Death (PLST A25-96)
- [x] Dragonskull Summit (DMC 207)
- [x] Sulfurous Springs (DMU 381)
- [x] Isolated Chapel (LTC 318)
- [x] Up the Beanstalk (WOE 195)
- [x] Rhythm of the Wild (RVR 217)
- [x] Bedevil (OTP 37)
- [x] Lathliss, Dragon Queen (J25 74)
- [x] Leyline Tyrant (TDC 221)

---

## 📦 Package 820924: Journeys End Games
**Status:** ✅ Arrived  
**Items:**
- [x] Crumbling Necropolis (DDH 74)
- [x] Overgrown Tomb (RTR 243)
- [x] Silumgar, the Drifting Death (FRF 157)
- [x] Elemental Bond (ORI 174)
- [x] Bladewing the Risen (C17 163)
- [x] Scalelord Reckoner (C17 6)
- [x] Rivaz of the Claw (DMU 314)
- [x] Stomping Ground (RVR 289)
- [x] Escape to the Wilds (EOC 115)

---

## 📦 Package 820929: Markovs Emporium
**Status:** ✅ Arrived  
**Items:**
- [x] Exotic Orchard (MKC 260)
- [x] Steel Hellkite (MKC 239)
- [x] Mountain (FIN 305)
- [x] Plains (FIN 296)
- [x] Donnie's Bō / Shadowspear (PZA 17)
- [x] Sol Ring (TMC 59)

---

## 📦 Package 820931: Meta TCG
**Status:** ✅ Arrived  
**Items:**
- [x] Scion of the Ur-Dragon (TSP 246)
- [x] Scourge of Kher Ridges (FUT 107)
- [x] Dragonspeaker Shaman (PW25 3)
- [x] Reflecting Pool (EOS 36)

---

## 📦 Package 820927: Spellfinder
**Status:** ✅ Arrived  
**Items:**
- [x] Drowned Catacomb (M11 224)
- [x] Blood Crypt (RTR 238)
- [x] Windswept Heath (KTK 248)
- [x] Sarkhan's Triumph (DTK 154)
- [x] Hellkite Charger (MM2 118)
- [x] The Ur-Dragon (C17 48)
- [x] Scourge of Valkas (IMA 145)
- [x] Colossal Majesty (M19 173)
- [x] Dragonlord Silumgar (2X2 203)
- [x] Two-Headed Hellkite (DMC 14)

---

## 📦 Package 820925: sugimo
**Status:** ✅ Arrived  
**Items:**
- [x] Austere Command (M3C 167)
- [x] Morophon, the Boundless (M3C 157)
- [x] Rishkar's Expertise (M3C 411)
- [x] Faeburrow Elder (BLC 252)
- [x] Farseek (BLC 119)
- [x] Sunpetal Grove (BLC 335)
- [x] Woodland Cemetery (BLC 354)
- [x] Garruk's Uprising (FDN 220)
- [x] Ramos, Dragon Engine (FDN 678)
- [x] Chaos Warp (TDC 208)
- [x] Dragon's Hoard (TDC 317)
- [x] Haven of the Spirit Dragon (TDC 370)
- [x] Arcane Sanctum (FIC 373)
- [x] Crux of Fate (FIC 275)

---

## 📦 Package 820926: The Feisty Goblin
**Status:** ✅ Arrived  
**Items:**
- [x] Dragonlord Kolaghan (DTK 218)
- [x] Cultivate (LTC 236)
- [x] Sulfur Falls (LTC 333)
- [x] Savage Ventmaw (FDN 665)
- [x] Command Tower (ECC 60)

---

## 📦 Package 820923: TinyMaru TCG
**Status:** ✅ Arrived  
**Items:**
- [x] Nature's Lore (DMR 170)
- [x] Three Visits (WHO 235)
- [x] Clifftop Retreat (PIP 258)
- [x] Guardian Project (PIP 199)
- [x] Adarkar Wastes (BLC 291)
- [x] Generous Gift (BLC 106)
- [x] Glacial Fortress (BLC 307)
- [x] Forest (J25 93) x2
- [x] Island (J25 84)
- [x] Swamp (J25 87)
- [x] Hinterland Harbor (DRC 160)
- [x] Beast Within (TDC 249)
- [x] Dragon Tempest (TDC 94)
- [x] Dragonlord Dromoka (TDC 286)
- [x] Dragonlord's Servant (TDC 210)
- [x] Hellkite Courser (TDC 218)
- [x] Sarkhan, Soul Aflame (TDC 301)
- [x] Temur Ascendancy (TDC 305)
- [x] Blasphemous Act (EOC 86)
- [x] Shivan Reef (EOC 179)
- [x] Breeding Pool (EOE 251)
- [x] Jungle Shrine (ECC 154)
- [x] Kodama's Reach (ECC 113)
- [x] Return of the Wildspeaker (ECC 115)
- [x] Sandsteppe Citadel (ECC 164)
- [x] Savage Lands (ECC 165)
- [x] Terminate (ECC 134)

---

# Order Tracking: Ur-Dragon (Order #235644)

**Order Date:** 2026-03-10  
**Status:** 1/2 Packages Received

---

## 📦 Package #235644-865060: Spellfinder
**Status:** ⏳ Pending  
**Items:**
- [ ] Arid Mesa (ZEN 211)
- [ ] Godless Shrine (GTC 242)
- [ ] Flooded Strand (KTK 233)
- [ ] Scalding Tarn (MM3 244)
- [ ] Watery Grave (GRN 259)
- [ ] Indatha Triome (IKO 309)
- [ ] Savai Triome (IKO 253)
- [ ] Ketria Triome (IKO 250P)
- [ ] Swiftfoot Boots (KHC 105)
- [ ] Verdant Catacombs (MH2 479)
- [ ] Jetmir's Garden (SNC 250)
- [ ] Raffine's Tower (SNC 254)
- [ ] Spara's Headquarters (SNC 257)
- [ ] Xander's Lounge (SNC 358)
- [ ] Lightning Greaves (PIP 233)
- [ ] Bloodstained Mire (MH3 216)
- [ ] Polluted Delta (MH3 224)

---

## 📦 Package #235644-865061: Fox & Fable Games
**Status:** ✅ Arrived  
**Items:**
- [x] Steam Vents (GRN 257)
- [x] Hallowed Fountain (RNA 251)
- [x] Raugrin Triome (IKO 251)
- [x] Zagoth Triome (IKO 313)
- [x] Marsh Flats (MH2 248)
- [x] Dragon's Hoard (AFC 204)
- [x] Ziatora's Proving Ground (SNC 261)
- [x] Sacred Foundry (EOE 256)
